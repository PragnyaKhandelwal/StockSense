...
{
    if (symbol != m_currentSymbol || prices.size() < 10)
        return;
    ...
    connect(reply, &QNetworkReply::finished, [this, reply, symbol]()
            {
                if (reply->error() == QNetworkReply::NoError)
                {
                    ...
                    if (!doc.isNull())
                    {
                        ...
                        if (prices.size() > 0)
                        {
                            m_lastFetchedPrice = prices.last();
                            cacheStock(symbol, prices);
                            m_historicalData = prices;
                            analyzeWithAllDSA();
                            update();
                        ...
                        {
                            m_trendDirection = "nodata";
                            update();
                        }
                    }
                }
                // (Optional: else block for errors, or logging)
            });
...
QVector<double> PredictionChartWidget::parseYahooData(const QJsonObject &response)
    ...
    void PredictionChartWidget::detectTrendWithStack()
    {
        ...
        if (trend > 5.0)
            m_trendDirection = "bullish";
        else if (trend < -5.0)
            m_trendDirection = "bearish";
        else
            m_trendDirection = "sideways";
    }
    ...
    void PredictionChartWidget::paintEvent(QPaintEvent * event)
    {
        ...
        if (m_historicalData.isEmpty())
        {
            ...
            {
                msg = QString("📊 Select a stock from the dropdown to begin analysis");
            }
            else if (m_trendDirection == "loading")
            {
                msg = (m_lastFetchedPrice > 0) ? QString("Live Price for %1: ₹%2\n\nFetching historical data...")
                                                     .arg(m_currentSymbol)
                                                     .arg(m_lastFetchedPrice, 0, 'f', 2)
                                               : QString("Fetching live price...");
            }
            else
            {
                msg = QString("❌ No data for %1\n\nTry: RELIANCE, TCS, INFY, HDFCBANK, BHARTIARTL").arg(m_currentSymbol);
            }
